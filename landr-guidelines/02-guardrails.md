...
It applies regardless of which system (LandR or SpaDES) is under focus. For the short
action checklist, see `00-user-setup.md`.

> **Shared AGENTS.md template.** A template `AGENTS.md` lives in the LandR.ai repo
> (https://github.com/CeresBarros/LandR.ai, at `AGENTS.md`). Usage: copy it into the
...
|---|---|---|---|
| **`AGENTS.md`** (memory) | Project root, trusted workspace only | Prose context the model reads every conversation | Project conventions, ecosystem map, coding standards |
...
`~/.posit/assistant/hooks/`) and referenced from the global `settings.json`.
...
> For the exact files, values, and install paths used on a specific machine, a local,
> machine-specific **instance** of this doc is kept rather than putting them in this
> shared document — see the **"Local instances of these guidelines"** section in
> `00-user-setup.md` for the naming and handling convention.
> `02a-implemented-reference.local.md` is the local instance for the original setup (a
> machine-specific record kept out of the shared guidelines, not part of the template).
