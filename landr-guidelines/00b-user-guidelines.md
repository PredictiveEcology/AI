...
LandR/SpaDES. It has two parts: (1) using the assistant responsibly for ecological-model
development, and (2) day-to-day tips and tools for getting good results.

> **Which assistant this assumes.** This guidance assumes you're running Posit Assistant
> in Positron. Slash commands (e.g. `/clear`, `/compact`, `/savememory`) and UI details may
...
> **You are responsible for what the assistant does.** The AI acts on your behalf, but
> the responsibility for its actions and outputs is yours — including code it writes,
> changes it makes, and the instructions it follows (the assistant-facing `01`–`03`
> guidelines, `AGENTS.md`, hooks, and skills). Knowing what the assistant is instructed to
> do is part of that responsibility: review those instructions, and the diffs and results
> it produces, before relying on them.

## Using the assistant for ecological modeling

LandR work is ecological-model development, and the assistant is a tool for *building and
checking* those models — not for originating the science. That's your job. The points below
adapt the seven guidelines of Ferrari et al. (2026) — a viewpoint on responsible generative-AI
use in the ecological sciences — to this workspace. They lean on rules already implemented in
`AGENTS.md` (caching/reproducibility, respecting existing methods, the AI-authorship footer)
rather than restating them.

- **Lead with the ecology, then reach for the assistant.** Ground module design and
  research questions in ecological theory and field knowledge; use the assistant to
  implement and verify, not to invent the hypotheses or the model structure.
- **Prefer the simplest sufficient tool.** Don't reach for AI-driven complexity where an
  existing LandR/SpaDES function, a plain script, or a standard statistical method already
  does the job — consistent with the workspace rule to respect existing methods and style.
- **The fundamentals of inference still apply.** When you have the assistant wrangle or
  combine data (e.g. cover layers from different sources, or merging species and ecoregion
  tables), keep sampling design, units, and metadata in view; treat merges of unknown
...
- **Keep yourself in the loop and keep building skill.** Read and understand the module
  code the assistant produces rather than accepting black-box output; this pairs with the
...
| --- | --- | --- |
| Provide verification criteria | "add a helper to validate species codes" | "add a helper to validate species codes; example cases: `Pinu_con` valid, `""` invalid. Run the testthat tests after." |
| Verify against a run | "make `Biomass_core` run" | "run `simInit()`/`spades()` on the test setup, show the error, fix the root cause, and rerun to confirm." |
...
| Strategy | Weak prompt | Stronger prompt |
| --- | --- | --- |
| Scope the task | "add tests for the module" | "write a testthat test for `<fn>` covering the empty-cohort edge case; build a small in-memory `data.table`." |
| Point to sources | "why is this parameter here?" | "look through this module's git history and summarize how `sppEquiv` came to be a parameter." |
| Reference existing patterns | "add a new data-prep module" | "look at `Biomass_speciesData` for the data-prep pattern and follow it for the new module." |
...
| `/compact` | Mid-task when context is filling; optionally `/compact <focus>` to steer the summary, e.g. `/compact focus on the Biomass_core parameter changes`. |
...
`sppEquiv`", and its exploration stays out of your main conversation.
...
| Prompt examples (points 2–3, 7) | n/a | n/a | Pure prompt-writing advice — fully generic across assistants; only the LandR/SpaDES content is workspace-specific. |
