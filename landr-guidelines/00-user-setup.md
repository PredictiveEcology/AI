...
This is your short, action-oriented checklist for setting up a Posit-Assistant workspace
for LandR **or** SpaDES development. It groups the setup steps; the reference docs (`01`–`03`)
...
> reference the AI consults when a step needs detail (the `→ detail:` pointers). They are
> not required reading to *complete setup*, but they define what the assistant is
> instructed to do — you remain responsible for what the assistant does on your behalf, so
> review them whenever you want to know or adjust that.
...
> to work with the assistant once it's running — responsible use for ecological modeling,
> prompting habits, context management, and which commands transfer to other tools — see
> `00b-user-guidelines.md`.

## Steps

**Steps 1–2 are your decisions; steps 3–6 you can hand to the assistant** — see the
...
0. **Before you start.** Have the software installed: **Positron** with **Posit
   ...
   **GitHub** account. Without git/GitHub you can still download this repo (and the LandR
   repos) as a zip from GitHub; GitHub is not needed to *use* LandR. No R versions or R
   package lists to manage — LandR's own tooling and the assistant handle package setup.

1. **Choose and confirm the project folder.** Decide the working-directory path that
   collects your module/package repos side by side. Note any sibling reference folder
   (e.g. a SpaDES toolkit folder) and whether the assistant may read outside the project.
   → detail: `01-project-setup.md`.

2. **Confirm the repository set and fork situation.** List which module families and
   accessory packages are in scope (don't assume a canonical set), and record for each
   repo whether it is an upstream clone or a personal fork, plus the default branch.
   → detail: `01-project-setup.md`.

3. **Create project memory and install guardrails.** Ask the assistant to add an
   `AGENTS.md` at the project root (from the team template) and to set up the two hook
   ...
   changes take effect in a **new conversation**, so be sure to save your progress and restart
...
4. **Get the shared skills.** Ask the assistant to clone or sync **both** sister skills
   repos — `CeresBarros/LandR.ai` (LandR skills) and `CeresBarros/SpaDES.ai` (SpaDES
   toolkit skills, used by default in LandR work) — into a local skills path (it needs
   permission to run `git` and write there); you decide the target path and confirm the
   skills are discoverable. Only create a new skill if a need isn't already covered.
   → detail: `03-skills.md`.
...
| 4 · Skills | "Fetch the shared team skills from both sister repos (LandR.ai and SpaDES.ai) into a local skills directory the assistant discovers, and confirm they are available." | "Clone (or sync) the `skills/` from both `CeresBarros/LandR.ai` and `CeresBarros/SpaDES.ai` into `~/.agents/skills`, then confirm `landr-overview` and `spades-overview` are discoverable." |
...
The skills come from **two sister repos** — `CeresBarros/LandR.ai` (LandR skills) and
`CeresBarros/SpaDES.ai` (SpaDES toolkit skills, used by default in LandR work); see
`03-skills.md`. `~/.agents/skills` is the tool-neutral, cross-tool skills location (read
by Posit Assistant and other Agent-Skills-aware tools), which is why it's used here; for
a Posit-Assistant-only install, use `~/.posit/assistant/skills` instead. Both are
...
- **Keep them out of the shared guidelines.** Local instances sit alongside the workspace
  ...
  enter the artifact everyone shares. The `.local.md` suffix marks them as such. This
  guidelines folder is **not currently a git repo**; if/when the shared guidelines move
  into one, add a `*.local.md` rule to its `.gitignore` to enforce the exclusion
  automatically. Until then, the exclusion is by convention — just don't copy `*.local.md`
  files in with the shared guidelines.
...
> **back up** into the
> shared path-agnostic doc — otherwise useful knowledge gets stranded where teammates
> never see it.
