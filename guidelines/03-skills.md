...
Get these before writing new ones; they are the starting point regardless of which
ecosystem (LandR or the SpaDES toolkit) is under focus. Ecosystem-agnostic skills (no
prefix) apply to both; `landr-*` skills are LandR-specific; `spades-*` skills are SpaDES
toolkit-specific.

**Shared (ecosystem-agnostic):**

| Skill | Purpose |
|---|---|
| `testing` | Unit and integration tests for modules, toolkit packages, and accessory packages (LandR/LandR.CS/fireSenseUtils). |
| `code-documentation` | Comments, module metadata `desc` fields, roxygen2, NEWS — across both ecosystems. |
...
| `landr-code-development` | Implementing/changing simulation processes and LandR-specific mechanisms (cohorts, pixel groups, species/ecoregion); builds on `spades-module-development`. |
...
**SpaDES toolkit skills** — used **by default** in LandR work, alongside the LandR skills:
...
| `spades-package-development` | Developing the SpaDES toolkit packages themselves (APIs, NAMESPACE, devtools, versioning). **Stub, not yet populated.** |

For LandR tasks, the LandR skills take precedence where a topic overlaps with a SpaDES
skill (e.g. `landr-code-development` over `spades-module-development` for process logic);
otherwise use the SpaDES skill for toolkit-general concerns.
...
given machine. The shared team skills are published remotely and then synced or cloned
into a local skill path.
...
2. Pick a clear, unique `name` and matching directory. Prefix `landr-` or `spades-` only
   when the skill is genuinely specific to that ecosystem; leave unprefixed if it applies
   to both (or to other PredictiveEcology team work, e.g. data-viz or report-writing).
...
4. Ground the body in **real file paths and short excerpts** from the system under
   focus.
5. Offload long catalogs (function lists, module families, package listings) to
   `references/`.
