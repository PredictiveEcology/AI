...
It applies regardless of which ecosystem (LandR or the SpaDES toolkit) is under focus —
the reference set below is ecosystem-agnostic and serves both unchanged. For the short
action checklist, see `00-user-setup.md`.

> **Shared AGENTS.md template.** A template `AGENTS.md` lives in the `AI` repo
> (https://github.com/PredictiveEcology/AI, at `AGENTS.md`). Usage: copy it into the
...
|---|---|---|---|
| **`AGENTS.md`** (memory) | Project root, trusted workspace only | Prose context the model reads every conversation | Project conventions, ecosystem/toolkit map, coding standards |
